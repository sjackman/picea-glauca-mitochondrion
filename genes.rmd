...
family_colour <- c(
	"#7A6BCA", "#87D94A", "#D0592C", "#7BC3C3",
	"#475333", "#D0B245", "#CE559B", "#C8A597",
	"#404B61", "#90D798", "#CA53D4", "#CB5261",
	"#703929", "#648D3E", "#9C9ED1", "#683360")
```
...
scaffold_sizes <- read_tsv("pg29mt-scaffolds.fa.fai",
	col_names = c("Scaffold", "Size", "Pos", "Line", "Stride")) %>%
...
# Gene order
```{r gene-order, fig.width = 7, fig.height = 9, dpi = 300}
data_genes <- data %>%
	filter(!Family.short %in% ignore_families) %>% droplevels %>%
	mutate(
...
ggplot(data_genes) +
	...
	scale_fill_manual("", values = family_colour) +
